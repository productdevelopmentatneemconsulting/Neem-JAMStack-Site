...
{data.allSanityService.edges.map(client=>(
    ...
    <Grid item md={4}xs={12} className="card-mid-page"  key={client.node._id}>
...
<BigHomeCard url={detail.node.redirectLink} tagline={detail.node.tagline} title={detail.node.headerText} img={detail.node.imageShow.asset.fluid} text={detail.node._rawDescription}/>
  </div>
))}
</Grid>
...
{data.allSanityClient.edges.map(client=>(
    ...
    <div style={{margin:"20px"}}key={client.node.id} >
   <Link to={'/project/'+client.node.slug.current+''}> <CardSmall  title={client.node.name} img={client.node.preview.asset.fluid}/>
   </Link>
